...
빌드 없음. `index.html`을 브라우저로 열면 데모 데이터로 바로 돌아갑니다.

**내 실제 에이전트 + 실시간 상태로 보려면** (Node 18+):

```bash
node scan.mjs    # ~/.claude/agents, ~/.codex/prompts 스캔 → agents.json
node serve.mjs   # http://127.0.0.1:4173 (+ /api/status 실시간 상태)
```

Claude Code 세션이 돌면 해당 에이전트 모니터에 불이 들어옵니다 (10초 폴링, 5분/30분 룰).
...
| `docs/ROADMAP.md` | **SSOT** — MVP 정의, 마일스톤 M0~M3, 핸드오프 프로토콜 |
| `docs/HANDOFF.md` | 설계 의도, 좌표계, 데이터 모델, 렌더 파이프라인 |
| `docs/OPUS-BRIEF.md` | 다음 단계(실데이터 연동) 요구사항 명세 + Opus 지시 프롬프트 |

라이브 데모: https://leeyoojinwork-pm.github.io/agent-hq/
