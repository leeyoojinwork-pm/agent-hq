...
```
너는 Agent HQ 프로젝트를 이어받는다. 리포의 docs/ROADMAP.md → docs/HANDOFF.md → docs/OPUS-BRIEF.md 순서로 먼저 읽어라.
ROADMAP.md가 SSOT다: 체크박스의 첫 번째 미완료 항목이 네 현재 작업이고, 하단 "인계 메모"에 직전 작업자의 상태가 있다.
